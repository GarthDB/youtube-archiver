...
class DefaultArchivingService(ArchivingService):
    ...
    async def process_all_channels(self) -> BatchProcessingResult:
        ...
        try:
            ...
            async def process_channel_with_semaphore(
                channel_config: Any,
            ) -> ChannelProcessingResult:
    ...
    async def process_channel(self, channel: Channel) -> ChannelProcessingResult:
        ...
        try:
            ...
            eligible_videos = [
                video
                for video in videos
                if video.is_eligible_for_archiving(age_threshold_hours)
            ]
            ...
            logger.info(
                f"Found {len(eligible_videos)} eligible videos in {channel.name}"
            )

            # Process eligible videos
            if dry_run:
                logger.info(
                    f"DRY RUN: Would process {len(eligible_videos)} videos in {channel.name}"
                )
                ...
                for i in range(0, len(eligible_videos), batch_size):
                    ...
                    # Process batch
                    batch_results = (
                        await self.visibility_manager.change_visibility_batch(
                    ...
                    for result in batch_results:
                        ...
                        if result.is_success:
                            logger.debug(
                                f"Successfully processed video: {result.video.title[:50]}..."
                            )
                        else:
                            logger.warning(
                                f"Failed to process video: {result.video.title[:50]}... - {result.error_message}"
                            )
            ...
            for channel_config in enabled_channels:
                ...
                try:
                    ...
                    eligible_videos = [
                        v
                        for v in videos
                        if v.is_eligible_for_archiving(age_threshold_hours)
                    ]
                    ...
                    summary["total_videos"] = summary.get("total_videos", 0) + len(
                        videos
                    )
                    summary["eligible_videos"] = summary.get(
                        "eligible_videos", 0
                    ) + len(eligible_videos)

                except Exception as e:
                    logger.error(
                        f"Error getting summary for channel {channel_config.name}: {e}"
                    )
    ...
    def validate_configuration(self) -> list[str]:
        ...
        try:
            ...
            if age_threshold < 1 or age_threshold > 168:
                errors.append(
                    f"Invalid age threshold: {age_threshold} hours (must be 1-168)"
                )
            ...
            if max_videos < 1 or max_videos > 500:
                errors.append(
                    f"Invalid max videos per channel: {max_videos} (must be 1-500)"
                )
            ...
            for channel in channels:
                ...
                if channel.max_videos_to_check < 1 or channel.max_videos_to_check > 500:
                    errors.append(
                        f"Invalid max videos for {channel.name}: {channel.max_videos_to_check}"
                    )
    ...
    async def _check_permissions_batch(self, videos: list[Video]) -> dict[str, bool]:
        ...
        for video in videos:
            try:
                permissions[video.id] = await self.visibility_manager.can_modify_video(
                    video.id
                )
            except Exception:
